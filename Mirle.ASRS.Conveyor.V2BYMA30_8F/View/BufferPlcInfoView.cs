...
namespace Mirle.ASRS.Conveyor.V2BYMA30_8F.View
{
    public partial class BufferPlcInfoView : Form
    {
            ...
            try
            {
                if (conveyorController.IsConnected)
                {
                    if (comboBoxBufferIndex.SelectedIndex != -1)
                    {
                        ...
                        clsTool.Signal_Show(conveyorController.GetBuffer(StnIdx).Cylinder, ref label_Cylinder);
                        clsTool.Signal_Show(conveyorController.GetBuffer(StnIdx).Empty, ref label_Empty);

                        clsTool.Signal_Show(conveyorController.GetBuffer(StnIdx).IniACK, ref lblIni_Ack);
                        clsTool.Signal_Show(conveyorController.GetBuffer(StnIdx).PathFinishACK, ref lblIniPathFinish_Ack);
                        ...
                        */
                        ...
                        clsTool.Signal_Show(conveyorController.GetBuffer(StnIdx).IniReq_PC, ref lblIni);
                        clsTool.Signal_Show(conveyorController.GetBuffer(StnIdx).PathFinish_PC, ref lblIniPathFinish); 
        ...
        private void btnInitial_PcToPlc_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                    ...
                    conveyorController.IsConnected)
                {
                    ...
                    conveyorController.GetBuffer(StnIdx).PathFinish();
        ...
        private void btnBCR_Check_Request_Click(object sender, EventArgs e)
        {
            ...
            {
                int StnIdx = Convert.ToInt32(comboBoxBufferIndex.Text.Split(':')[0]);
                conveyorController.GetBuffer(StnIdx).WriteBcrSetReadReq(1);
            }
        }
    }
}
