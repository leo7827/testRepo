...
namespace Mirle.ASRS.Conveyor.V2BYMA30_8F.Signal
{
    public class SignalMapper
    {
        ...
        {
            new BlockInfo(new DDeviceRange("D1001", "D5015"), "ReadWrite", 0),
        };
        ...
        private void MappingBuffer()
        {
            ...
            for (int i = 1; i <= BufferCount; i++)
            {
                ...
                bufferSignal.StatusSignal = new BufferStatusSignal
                {
                    ...
                    Cylinder = new Bit(_mplc, $"D{startAddress + offset + 3}.A"),
                    Empty = new Bit(_mplc, $"D{startAddress + offset + 3}.B")
                };
                ...
                //V 1.0.0.5 改成用bit顯示
                bufferSignal.AckSignal.IniSignalACK = new BufferIniSignalAck
                {
                    IniACK = new Bit(_mplc, $"D{startAddress + offset + 10}.0"),
                    PathFinishACK = new Bit(_mplc, $"D{startAddress + offset + 10}.1"),
                };                
                ...
                //V 1.0.0.5 改成用bit顯示
                bufferSignal.RequestController.IniSignalReq = new BufferIniSignal
                {
                    IniReq = new Bit(_mplc, $"D{controllerStartAddress + offset + 10}.0"),
                    PathFinish = new Bit(_mplc, $"D{controllerStartAddress + offset + 10}.1"),
