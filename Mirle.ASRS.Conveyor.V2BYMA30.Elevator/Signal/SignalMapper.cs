...
namespace Mirle.ASRS.Conveyor.V2BYMA30_Elevator.Signal
{
    public class SignalMapper
    {
        ...
        {
            new BlockInfo(new DDeviceRange("D1001", "D4000"), "ReadWrite", 0),
        };
        ...
        private void MappingConveyor()
        {
            ...
            _Signal.Floor = new Word(_mplc, $"D{addr + 3}");
            //_Signal.RunningSts = new Word(_mplc, $"D{addr + 4}");

            _Signal.EleStatus = new ElevatorStatusSignal
            {
                AgvMode = new Bit(_mplc, $"D{addr + 4}.0"),
                PlatformOff = new Bit(_mplc, $"D{addr + 4}.1"),
                PlatformOn = new Bit(_mplc, $"D{addr + 4}.2"),
                Idle = new Bit(_mplc, $"D{addr +  4}.3"),
                Running = new Bit(_mplc, $"D{addr +  4}.4"),
                Up = new Bit(_mplc, $"D{addr + 4}.5"),
                Down = new Bit(_mplc, $"D{addr +  4}.6"),
            };


            _Signal.ModeStatus = new Word(_mplc, $"D{addr + 6}");
            _Signal.DoorStatus = new Word(_mplc, $"D{addr + 7}");
            ...
            _Signal.OpcData = new WordBlock(_mplc, $"D{addr + 1}", 2050);
            ...
            _Signal.Controller = new ConveyorControllerSignal
            {
                ...
                ModeChange = new Word(_mplc, $"D{addr + 6}"),
                Path = new Word(_mplc, $"D{addr + 8}"),
                DoorNoticce = new Word(_mplc, $"D{addr + 9}"),
