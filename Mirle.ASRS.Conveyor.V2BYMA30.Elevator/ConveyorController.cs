...
using Mirle.ASRS.Conveyor.V2BYMA30_Elevator.View;
...
namespace Mirle.ASRS.Conveyor.V2BYMA30_Elevator
{
    public class ConveyorController : IDisposable
    {
        ...
        private ThreadWorker _Door;
        private ThreadWorker _Floor;
        ...
        public bool AgvMode => Signal.EleStatus.AgvMode.IsOn();
        public bool PlatformOn => Signal.EleStatus.PlatformOn.IsOn();
        public bool PlatformOff => Signal.EleStatus.PlatformOff.IsOn();

        //conveyorController.Signal.DoorStatus.GetValue().ToString();


        public bool Idle => Signal.EleStatus.Idle.IsOn();
        public bool Running => Signal.EleStatus.Running.IsOn();
        public bool Up => Signal.EleStatus.Up.IsOn();
        public bool Down => Signal.EleStatus.Down.IsOn();
        ...
        public bool WriteDoorIndex(int index)
        {
            var ctrl = _Signal.GetConveyorSignal().Controller;
            try
            {
                ctrl.DoorNoticce.SetValue(index);
        ...
        public bool SetFloor(int iPath)
        {
            var ctrl = _Signal.GetConveyorSignal().Controller;
            try
            {
                ctrl.Path.SetValue(iPath);
        ...
        public void Door_Ini()
        {
            if (!IsConnected)
                return;

            var ctrl = _Signal.GetConveyorSignal().Controller;
            ctrl.DoorNoticce.SetValue(0);

        }


        public void Door_Clear()
        {
            if (!IsConnected)
                return;
            var plc = _Signal.GetConveyorSignal();
            var ctrl = _Signal.GetConveyorSignal().Controller;
            if (plc.DoorStatus.GetValue() == ctrl.DoorNoticce.GetValue())
            {
                ctrl.DoorNoticce.SetValue(0);
            }
        }

        public void Floor_Ini()
        {
            if (!IsConnected)
                return;

            var ctrl = _Signal.GetConveyorSignal().Controller;
            ctrl.Path.SetValue(0);

        }

        public void Floor_Clear()
        {
            if (!IsConnected)
                return;
            var plc = _Signal.GetConveyorSignal();
            var ctrl = _Signal.GetConveyorSignal().Controller;
            if (plc.Floor.GetValue() == ctrl.Path.GetValue())
            {
                ctrl.Path.SetValue(0);
            }
        } 
